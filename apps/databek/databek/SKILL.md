---
name: databek
description: App-level AI contract for the Databek platform. Read this after RULES.md to understand the app's shape, the build loop, and where artifacts live before invoking any skill.
---

# Databek — app-level contract

Databek is an empty, AI-extensible internal-tools platform on **Frappe v15**
...
| Artifact            | Path                                                        |
|---------------------|-------------------------------------------------------------|
| App manifest        | `databek/hooks.py` (markers per section)                       |
| Modules list        | `databek/modules.txt`                                          |
| DocType             | `databek/<module>/doctype/<name>/<name>.json` + `.py` (+`.js`) |
| Fixtures (frozen)   | `databek/fixtures/*.json`                                      |
| Changelog           | `databek/CHANGELOG.md`                                         |
| Module AI contract  | `databek/<module>/SKILL.md`                                    |
