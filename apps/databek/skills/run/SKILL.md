...
The first `up` also creates the empty site and installs `databek`. Later runs just
...
docker compose exec backend bench --site databek.localhost migrate
docker compose exec backend bench build --app databek
docker compose exec backend bench --site databek.localhost clear-cache
```
