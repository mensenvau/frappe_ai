...
- [`../../RULES.md`](../../RULES.md)
- Existing `databek/fixtures/workspace.json` (if any) to extend rather than clash.
- The DocType(s) being surfaced.

## Produces / edits

- A **Workspace** record (module `Databek`) — from `workspace.json.template`.
- A **Dashboard Chart** / **Number Card** record (module `Databek`).
- A **Report** — `databek/<module>/report/<name>/` (Query/Script) or a Report record.
...
2. Create the record(s) with `module = "Databek"`.
