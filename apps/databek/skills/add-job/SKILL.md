...
- [`../../RULES.md`](../../RULES.md)
- [`../../databek/hooks.py`](../../databek/hooks.py) — `scheduler_events` markers.

## Produces / edits

- `hooks.py` `scheduler_events` between the `# >>> DATABEK:scheduler_events` markers.
- A task function in `databek/<module>/tasks.py`.
- (Async) a `frappe.enqueue(...)` call inside a controller/API.

## Procedure

1. Read RULES.md.
2. Implement the task function in `databek/<module>/tasks.py`.
3. **Scheduled:** add it to `scheduler_events` between the markers
   (`hourly`/`daily`/`weekly`/`monthly`, or `cron` with a crontab string).
4. **Async:** call `frappe.enqueue("databek.<module>.tasks.fn", **kwargs)` from
