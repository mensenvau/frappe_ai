...
- For the dockerized stack: the app must persist in the image/volume — note that
  bind-mounted `databek` persists, but `get-app`'d apps land in the bench volume;
...
3. Install on the site: `bench --site databek.localhost install-app <app>`.
...
bench --site databek.localhost install-app app
bench --site databek.localhost migrate
```
