...
3. **Every change is written to disk** (because `developer_mode=1`) and frozen
   into `databek/fixtures/` via the `export-fixtures` skill, so the site is
   reproducible.
...
6. **After every change:** `export-fixtures` → `run` (migrate + build) →
   append one line to `databek/CHANGELOG.md` recording: *what · which skill ·
   which rule(s)*.
...
9. **AI layer (Claude API).** "AI for management everywhere" is a first-class,
   ...
   - All model calls go through one service (`databek/ai/` — a thin Anthropic
...
- **Product brand = "Databek"** (titles, public site, emails). `databek` is
  only the backend app package name.
- App name `databek` — lowercase, snake_case (Frappe requirement).
- Module folder = snake_case under `databek/`; module label in `modules.txt`.
...
- Keep the `# >>> DATABEK:<section>:start / <<< end` markers in `hooks.py` intact.
