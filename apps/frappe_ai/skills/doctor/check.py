#!/usr/bin/env python3
"""Frappe AI doctor — static health check of the frappe_ai app (no bench required).

Usage:
    python skills/doctor/check.py apps/frappe_ai
...
def check_hooks(app_pkg: Path) -> None:
    ...
    for section in ("doc_events", "scheduler_events", "fixtures"):
        starts = len(re.findall(rf"# >>> FRAPPEAI:{section}:start", src))
        ends = len(re.findall(rf"# <<< FRAPPEAI:{section}:end", src))
...
def main() -> int:
    root = Path(sys.argv[1] if len(sys.argv) > 1 else "apps/frappe_ai")
    app_pkg = root / "frappe_ai" if (root / "frappe_ai").is_dir() else root
