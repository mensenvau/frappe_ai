---
name: doctor
description: Pre-flight health check of the whole frappe_ai app before a run — DocType JSON validity, controllers import, hooks markers intact, fixtures consistency, and migration dry-run. Use before run, or when something is broken and you need a diagnosis.
---
...
3. **Hooks intact** — `hooks.py` parses; the `# >>> FRAPPEAI:*` marker pairs are
...
```bash
python skills/doctor/check.py apps/frappe_ai        # static checks (no bench needed)
docker compose exec backend bench --site frappe_ai.localhost migrate --dry-run  # opt.
```
