...
description: Freeze config records (roles, custom fields, property setters, workspaces, custom doctypes) to frappe_ai/fixtures/*.json so the site is reproducible. Run after any change that creates config data records. Driven by the fixtures list in hooks.py.
---
...
- [`../../RULES.md`](../../RULES.md)
- [`../../frappe_ai/hooks.py`](../../frappe_ai/hooks.py) — the `fixtures` list decides
  what gets exported.

## Produces / edits

- `frappe_ai/fixtures/*.json` — one file per fixture DocType
...
3. Review the diff — fixtures should contain only Frappe AI-owned records (filters
   keep core data out).
...
- Only Frappe AI-owned records — keep filters (`module = "Frappe AI"`, name lists)
