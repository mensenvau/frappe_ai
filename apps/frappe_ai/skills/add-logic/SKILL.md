...
- [`../../RULES.md`](../../RULES.md)
- The target `frappe_ai/<module>/doctype/<name>/<name>.py`.
- The module `SKILL.md` (invariants you must preserve).
- [`../../frappe_ai/hooks.py`](../../frappe_ai/hooks.py) if using `doc_events`.

## Produces / edits

- Edits the target controller `.py` (lifecycle methods / whitelisted funcs).
- OR edits `hooks.py` `doc_events` between the `# >>> FRAPPEAI:doc_events` markers.
...
5. For `doc_events`: add `"DocType": {"event": "frappe_ai.<...>.handler"}` between
