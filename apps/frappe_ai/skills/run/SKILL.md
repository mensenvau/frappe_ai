...
The first `up` also creates the empty site and installs `frappe_ai`. Later runs just
...
docker compose exec backend bench --site frappe_ai.localhost migrate
docker compose exec backend bench build --app frappe_ai
docker compose exec backend bench --site frappe_ai.localhost clear-cache
```
