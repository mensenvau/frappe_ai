...
- For the dockerized stack: the app must persist in the image/volume — note that
  bind-mounted `frappe_ai` persists, but `get-app`'d apps land in the bench volume;
...
3. Install on the site: `bench --site frappe_ai.localhost install-app <app>`.
...
bench --site frappe_ai.localhost install-app app
bench --site frappe_ai.localhost migrate
```
