...
- A **Custom Field** record (module = `Frappe AI`).
- And/or a **Property Setter** record (module = `Frappe AI`).
- These are exported to `frappe_ai/fixtures/custom_field.json` /
...
3. Ensure `module = "Frappe AI"` so fixtures filters pick it up.
...
- Custom Fields/Property Setters must carry `module = "Frappe AI"`.
