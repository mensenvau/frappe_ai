...
- [`../../RULES.md`](../../RULES.md)
- Existing `mason/fixtures/workspace.json` (if any) to extend rather than clash.
- The DocType(s) being surfaced.

## Produces / edits

- A **Workspace** record (module `Mason`) — from `workspace.json.template`.
- A **Dashboard Chart** / **Number Card** record (module `Mason`).
- A **Report** — `mason/<module>/report/<name>/` (Query/Script) or a Report record.
...
2. Create the record(s) with `module = "Mason"`.
