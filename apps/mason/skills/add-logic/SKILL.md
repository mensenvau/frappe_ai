...
- [`../../RULES.md`](../../RULES.md)
- The target `mason/<module>/doctype/<name>/<name>.py`.
- The module `SKILL.md` (invariants you must preserve).
- [`../../mason/hooks.py`](../../mason/hooks.py) if using `doc_events`.

## Produces / edits

- Edits the target controller `.py` (lifecycle methods / whitelisted funcs).
- OR edits `hooks.py` `doc_events` between the `# >>> MASON:doc_events` markers.
...
5. For `doc_events`: add `"DocType": {"event": "mason.<...>.handler"}` between
