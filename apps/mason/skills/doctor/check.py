#!/usr/bin/env python3
"""Mason doctor — static health check of the mason app (no bench required).

Usage:
    python skills/doctor/check.py apps/mason
...
def check_hooks(app_pkg: Path) -> None:
    ...
    for section in ("doc_events", "scheduler_events", "fixtures"):
        starts = len(re.findall(rf"# >>> MASON:{section}:start", src))
        ends = len(re.findall(rf"# <<< MASON:{section}:end", src))
...
def main() -> int:
    root = Path(sys.argv[1] if len(sys.argv) > 1 else "apps/mason")
    app_pkg = root / "mason" if (root / "mason").is_dir() else root
