...
description: Freeze config records (roles, custom fields, property setters, workspaces, custom doctypes) to mason/fixtures/*.json so the site is reproducible. Run after any change that creates config data records. Driven by the fixtures list in hooks.py.
---
...
- [`../../RULES.md`](../../RULES.md)
- [`../../mason/hooks.py`](../../mason/hooks.py) — the `fixtures` list decides
  what gets exported.

## Produces / edits

- `mason/fixtures/*.json` — one file per fixture DocType
...
3. Review the diff — fixtures should contain only Mason-owned records (filters
   keep core data out).
...
bench --site mason.localhost export-fixtures --app mason
```
...
- Only Mason-owned records — keep filters (`module = "Mason"`, name lists)
