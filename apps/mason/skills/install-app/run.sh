#!/usr/bin/env bash
# Install an external Frappe app onto the dockerized Mason site.
...
BRANCH="${3:-version-15}"
SITE="${4:-mason.localhost}"

# Guardrail: refuse ERPNext without an explicit override env var.
if [[ "$APP" == "erpnext" && "${MASON_ALLOW_ERPNEXT:-0}" != "1" ]]; then
  echo "[install-app] REFUSING to install ERPNext (platform is Frappe-only)."
  echo "[install-app] If you truly want it, re-run with MASON_ALLOW_ERPNEXT=1."
  exit 1
fi
