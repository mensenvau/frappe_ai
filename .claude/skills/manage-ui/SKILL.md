---
name: manage-ui
description: Manage existing UI in databek — list/inspect, edit, reorder, hide, or remove Workspaces (menus), Dashboard Charts, Number Cards, Reports, and List-view settings. Use for "change the menu", "move/rename this card", "hide that report", "what's on the dashboard", "remove this from the menu". For brand-new UI, /build covers it.
---
...
1. [`apps/databek/RULES.md`](../../../apps/databek/RULES.md) — §8 primitives only.
2. [`apps/databek/skills/add-ui/SKILL.md`](../../../apps/databek/skills/add-ui/SKILL.md) — the UI primitives + templates.

## Inspect (read-only)

List Databek-owned UI records on the live site:
...
docker compose -f docker/docker-compose.yml exec -T backend bash -lc '
  ...
  for dt in Workspace "Dashboard Chart" "Number Card" Report; do
    echo "== $dt =="; bench --site databek.localhost list-documents "$dt" 2>/dev/null || \
    bench --site databek.localhost execute frappe.client.get_list --kwargs "{\"doctype\":\"$dt\",\"filters\":{\"module\":\"Databek\"},\"fields\":[\"name\"]}"
  done'
```

(Or read the frozen JSON in `apps/databek/databek/fixtures/workspace.json` etc.)
...
`module = "Databek"`. Re-`export-fixtures` after.
