# Databek — Build Brief for Claude Code
...
> foundation**. `Databek` is a working name (metaphor: AI = databek, laying modules onto
> the Frappe foundation brick by brick). If you change the name, replace `databek`
...
Databek is an internal-tools platform built on top of Frappe that the AI extends
...
background jobs, auth/SSO, asset build — Frappe provides all of it. What Databek adds:
...
- A single custom Frappe app: **`databek`**.
...
```
databek-platform/                  # git repo
  docker/                        # frappe_docker compose + entrypoint
    docker-compose.yml
    .env
  apps/
    databek/                       # custom Frappe app (bench new-app databek)
      databek/
...
> 2. `bench new-app databek` — create the custom app, `install-app` it onto the site.
> 3. Set `developer_mode = 1`.
> 4. Inside the `databek` app create: `RULES.md` (the text above), a `SKILL.md`
